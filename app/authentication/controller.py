...
class AuthControllerBase:
    ...
    async def get_user_in_db(self, username: str) -> UserInDB | None:
        query = "SELECT * FROM auth_user WHERE username = ($1)"
        record: Record = await self.db.select_one(query, username)
        if not record:
            return None
...
class LoginController(AuthControllerBase):
    ...
    async def authenticate_user(self, username: str, password: str) -> UserInDB | None:
        user: UserInDB | None = await self.get_user_in_db(username)
        if not user:
            return None
...
class RefreshController(AuthControllerBase):
    async def refresh_tokens(self) -> TokenPair:
        ...
        user: UserInDB = await self.get_user_in_db(token_sub.username)
        if not user:
    ...
    async def validate_refresh_token(token) -> RefreshTokenData:
        try:
            ...
            return RefreshTokenData(username=payload["sub"])
