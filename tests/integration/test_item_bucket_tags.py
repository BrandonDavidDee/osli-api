...
class TestItemBucketTagCreate:
    item_id = 100
    url = f"/api/items/bucket/{item_id}/tags?source_id=1"

    def test_tag_create_missing_source_id(self, client):
        response = client.post(f"/api/items/bucket/{self.item_id}/tags")
...
class TestItemBucketTagDelete:
    item_id = 100
    tag_item_bucket_id = 9123
    url = f"/api/items/bucket/{item_id}/tags/{tag_item_bucket_id}?source_id=1"

    def test_tag_delete_missing_source_id(self, client):
        response = client.delete(
            f"/api/items/bucket/{self.item_id}/tags/{self.tag_item_bucket_id}"
        )
        assert response.status_code == 422
