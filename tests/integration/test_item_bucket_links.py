...
class TestItemBucketLinkCreate:
    item_id = 100
    url = f"/api/items/bucket/{item_id}/links?source_id=1"

    def test_item_link_create_missing_source_id(self, client):
        response = client.post(url=f"/api/items/bucket/{self.item_id}/links")
...
class TestItemBucketLinks:
    item_id = 100
    source_id = 1
    url = f"/api/items/bucket/{item_id}/links"

    def test_get_item_links_no_results(self, client, mock_db_select_many):
        mock_db_select_many.return_value = [
            {
                "id": self.item_id,
                "source_bucket_id": self.source_id,
                "title": "Foobar",
                "mime_type": "image/jpeg",
                "file_path": "images/some-image.jpg",
                "file_size": 123456789,
                "notes": None,
...
class TestItemBucketLinkDelete:
    item_id = 100
    item_link_id = 7777
    url = f"/api/items/bucket/{item_id}/links/{item_link_id}"
